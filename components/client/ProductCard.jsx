...
const StockBadge = React.memo(({ stock }) => {
  if (stock === 0) return (
    <span className="text-rose-400 bg-rose-500/10 border border-rose-500/20 text-[11px] font-bold px-2.5 py-1 rounded-full">Out of Stock</span>
  );
  if (stock <= 5) return (
    <span className="text-amber-400 bg-amber-500/10 border border-amber-500/20 text-[11px] font-bold px-2.5 py-1 rounded-full">Only {stock} left!</span>
  );
  return (
    <span className="text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 text-[11px] font-bold px-2.5 py-1 rounded-full">In Stock</span>
  );
});
    ...
    <div className="product-card-premium group">
      ...
      <div className="p-4 relative z-10">
        ...
        <Link
          ...
          className="w-full py-2.5 rounded-xl text-sm font-bold transition-all duration-300 border border-[#FF8C00]/30 text-[#FF8C00] hover:bg-[#FF8C00] hover:text-white hover:border-[#FF8C00] hover:shadow-[0_4px_16px_rgba(255,140,0,0.35)] flex items-center justify-center inline-block text-center"
        >
          View Details →
        </Link>
      </div>
...
export default React.memo(ProductCard, (p, n) => {
  ...
  const nId = n.product._id || n.product.id;
  return pId === nId &&
